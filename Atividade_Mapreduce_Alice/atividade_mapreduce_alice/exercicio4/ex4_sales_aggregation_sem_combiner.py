...
def mapreduce_sem_combiner(transactions: List[Dict[str, str]]) -> List[Tuple[str, float]]:
    mapped = [(t["category"], float(t["quantity"]) * float(t["price"])) for t in transactions]
...
if __name__ == "__main__":
    ...
    resultado_sem = mapreduce_sem_combiner(transactions)
    salvar_resultados(os.path.join(results_dir, "resultado_sem_combiner.csv"), resultado_sem)

    print("\n--- RESULTADO SEM COMBINER ---")
    for cat, total in resultado_sem:
